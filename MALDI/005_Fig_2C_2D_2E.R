## Script for Figures 2C-2E - Differential analyses comparing tumor with stroma, and comparing clusters within respective compartments
# R version 4.4.0
...
D <- ggplot(data = Stroma_df, aes(x = StromaLogFC, y = -log10(StromaPval), color = diffexpressed, 
  ...
  geom_text_repel(data = Stroma_df[!grepl("X", Stroma_df$delabel) & Stroma_df$StromaPval < 0.05,],
                  ...
                  max.overlaps = 20)+
  xlab("Log2 fold change")+ ylab("Significance -log10(P)")+
  scale_color_manual(name = "Significance", values = c('#DC0018', "grey", '#2D69A9'), 
