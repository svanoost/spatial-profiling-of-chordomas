## Script for clustering of stroma segments only
# R version 4.4.0
...
Chord_stro <- Chord_dataset[, pData(Chord_dataset)$Segment == 'TME']

# Coefficient of variation (CV) function
calc_CV = function(x){sd(x) / mean(x)}

#### UMAP of stroma segments ####
# Find genes with top coefficient of variation (CV)
# Calculate CV values
CV_data = assayDataApply(Chord_stro, elt = "log2_q3", MARGIN = 1, calc_CV)

# Save CV values:
saveRDS(CV_data, './analysis_files/CV_data_stroma.RData')
...
set.seed(1)
umap_all = umap(t(assayDataElement(Chord_stro[GOI, ], elt = "log2_q3")), n_neighbors = 5)

# Save UMAP
write.csv(umap_all, './output_files/1_UMAP_StromaSegments.csv')

# Reload UMAP
umap_all <- read.csv('./output_files/1_UMAP_StromaSegments.csv')
row.names(umap_all) <- umap_all$X

# Make a UMAP data frame
umap_df = data.frame(UMAP1 = umap_all[,2], UMAP2 = umap_all[,3],
                     Segment = pData(Chord_stro)[rownames(umap_all), 'Segment'],
                     Individual = pData(Chord_stro)[rownames(umap_all), 'Tags'],
                     Location = pData(Chord_stro)[rownames(umap_all), 'Location'],
                     SlideName = pData(Chord_stro)[rownames(umap_all), 'SlideName'],
                     Nuclei = pData(Chord_stro)[rownames(umap_all), 'Nuclei'])

#### Graph-based clustering of stroma segments ####
set.seed(1)
clust_graphbased = clusterRows(t(assayDataElement(Chord_stro[GOI, ], elt = "log2_q3")),
                                        SNNGraphParam(k = 2, cluster.fun = 'louvain', 
                                                      cluster.args = list(resolution = 0.16)))
umap_df$snn <- clust_graphbased
umap_df$snn <- as.character(umap_df$snn)

#### Differential gene expression analysis ####
# Add the graph-based clusters to the GeoMx dataset
pData(Chord_stro)$cluster <- clust_graphbased

# Perform the differential analysis, similar to that of Tumor vs Stroma
set.seed(1)
test_dataset = Chord_stro
...
r_test$result_category[r_test$Estimate > 0.8] = 'Non-inflamed'
r_test$result_category[r_test$Estimate < (-0.8)] = 'Inflamed'
...
r_test$top_genes <- "no"
r_test[1:21, "top_genes"] <- "yes"
r_test[r_test$Gene == "IGKC", "top_genes"] <- "no"
...
names(indiv_colors) = unique(pData(Chord_stro)$Tags)
...
#### Save the data ####
# DEG results for stroma clusters
write.table(r_test, file = "./output_files/DEGs_Stroma_008_cutoff_graph_based_clusters.txt", 
            sep = "\t", row.names = FALSE)

# GeoMx dataset with only stroma segments and their clusters
saveRDS(Chord_stro, file = "./analysis_files/5_Chord_stroma_clusters.RData")

# Data for the heatmap - Supplementary Figure 4
save(col_ann, DEGs, zscores, file = "./analysis_files/Suppl_Fig_4_Heatmap.RData")

# Data for the UMAPs - Figure 1C
save(umap_df, file = "./analysis_files/Fig_1C_UMAPs.RData")

# Data for the volcano plot - Figure 1D
save(r_test, file = "./analysis_files/Fig_1D_Volcanoplot.RData")
